...
type DonacionExtendida = (typeof mockDonaciones)[0]
...
export default function DonacionesPage() {
    ...
    const filtered = mockDonaciones.filter((d) => {
        const matchSearch =
    ...
    const pendientesPLD = mockDonaciones.filter((d) => d.requiere_reporte_pld && !d.reportada_pld).length
    const pendientesSAT = mockDonaciones.filter((d) => !d.reportada_sat).length
    const totalRequierenReporte = mockDonaciones.filter((d) => d.requiere_reporte_pld).length
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <Card className="border-l-4 border-l-primary">
                        <CardContent className="p-4">
                            ...
                            <p className="text-[11px] text-muted-foreground mt-1">{mockDonaciones.length} donaciones</p>
                        </CardContent>
                    </Card>
                    <Card className="border-l-4 border-l-warning">
                        <CardContent className="p-4">
                            ...
                            <p className="text-[11px] text-muted-foreground mt-1">≥ 645 UMAs ({formatMXN(645 * umaActual.valor)})</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-l-4 border-l-success">
                        <CardContent className="p-4">
                            ...
                            <p className="text-xl font-bold">{mockDonaciones.filter(d => d.reportada_sat).length}</p>
                        ...
                        <DialogContent className="sm:max-w-xl">
                            ...
                            <div className="flex flex-col gap-4 pt-2">
                                ...
                                <div className="rounded-md border border-warning/40 bg-warning/5 p-3 text-xs text-muted-foreground">
                                    ...
                                    {formatMXN(645 * umaActual.valor)} (645 UMAs × ${umaActual.valor})
                                </div>
                    ...
                    <CardHeader className="py-3 px-5">
                        <div className="flex items-center justify-between gap-4">
                            <CardTitle className="text-sm font-medium text-muted-foreground">
                                ...
                                {filtered.length !== mockDonaciones.length && ` filtradas de ${mockDonaciones.length}`}
                            </CardTitle>
                                    ...
                                    <TableRow key={don.donacion_id} className="cursor-pointer">
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {(() => {
                                                    const donante = mockDonantes.find(d => d.donante_id === don.donante_id)
                                        ...
                                        <TableCell className="hidden lg:table-cell text-xs text-muted-foreground">
                                            {umasEquivalentes(don.monto, don.valor_uma_aplicado)} UMAs
                                        </TableCell>
                            ...
                            <div className="flex flex-col gap-4 pt-2">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-xl font-bold mt-0.5">{formatMXN(detail.monto)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-muted-foreground">Equivalente UMA</p>
                                        <p className="text-sm font-medium mt-0.5">
                                            {umasEquivalentes(detail.monto, detail.valor_uma_aplicado)} UMAs
                                        </p>
